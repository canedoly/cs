...
void md5_init( md5_context_t* ctx ) {
	ctx->m_buf[ 0 ] = 0x67452301;
	ctx->m_buf[ 1 ] = 0xefcdab89;
	ctx->m_buf[ 2 ] = 0x98badcfe;
	ctx->m_buf[ 3 ] = 0x10325476;

	ctx->m_bits[ 0 ] = 0;
	ctx->m_bits[ 1 ] = 0;
}

void md5_update( md5_context_t* ctx, unsigned char const* buf, unsigned int len ) {
	unsigned int t;

	t = ctx->m_bits[ 0 ];

	if ( ( ctx->m_bits[ 0 ] = t + ( ( unsigned int )len << 3 ) ) < t )
		ctx->m_bits[ 1 ]++;
	...
	if ( t ) {
		unsigned char* p = ( unsigned char* )ctx->m_in + t;
		...
		memcpy( p, buf, t );
		md5_transform( ctx->m_buf, ( unsigned int* )ctx->m_in );
		buf += t;
		len -= t;
	}

	while ( len >= 64 ) {
		memcpy( ctx->m_in, buf, 64 );
		md5_transform( ctx->m_buf, ( unsigned int* )ctx->m_in );
		buf += 64;
		len -= 64;
	}

	memcpy( ctx->m_in, buf, len );
}
...
void md5_final( unsigned char digest[ MD5_DIGEST_LENGTH ], md5_context_t* ctx ) {
	...
	*p++ = 0x80;
	...
	if ( count < 8 ) {
		memset( p, 0, count );
		md5_transform( ctx->m_buf, ( unsigned int* )ctx->m_in );
		memset( ctx->m_in, 0, 56 );
	}

	else memset( p, 0, count - 8 );

	( ( unsigned int* )ctx->m_in )[ 14 ] = ctx->m_bits[ 0 ];
	( ( unsigned int* )ctx->m_in )[ 15 ] = ctx->m_bits[ 1 ];

	md5_transform( ctx->m_buf, ( unsigned int* )ctx->m_in );
	memcpy( digest, ctx->m_buf, MD5_DIGEST_LENGTH );
...
bool md5_compare( const md5_value_t& data, const md5_value_t& compare ) {
	return memcmp( data.m_bits, compare.m_bits, MD5_DIGEST_LENGTH ) == 0;
}

void md5_value_t::zero( ) {
	memset( this->m_bits, 0, sizeof( this->m_bits ) );
}